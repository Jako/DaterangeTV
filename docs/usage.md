...
If the output options for the custom template variable could not be set (i.e. inside of MIGX)
...
the grid column. The name option has to be filled with the template variable name prepended
with `tv_`.
...
be rendered with the `DaterangeTV.Renderer`. The format of the date can be
changed with the system setting `daterangetv.manager_format` and the separator
between two dates with the system setting `daterangetv.separator`. If you want
to use only one column, you have to fill the Collections snippet renderer option
with `Daterange`.

## System Settings

DaterangeTV uses the following system settings in the namespace `daterangetv`:

| Key                        | Name                | Description                                                                                                                                                              | Default                   |
|----------------------------|---------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
| daterangetv.debug          | Debug               | Log debug information in the MODX error log.                                                                                                                             | No                        |
| daterangetv.format         | Date Format         | A between day, month and year by &#124; separated list with [strftime](https://www.php.net/manual/en/function.strftime.php) formats.                                     | "%e. &#124; %B &#124; %Y" |
| daterangetv.manager_format | Manager Date Format | A between day, month and year by &#124; separated list with [Ext JS Date](https://docs.sencha.com/extjs/3.4.0/#!/api/Date) formats to format the Collections/MIGX value. | "j. &#124; F &#124; Y"    |
| daterangetv.separator      | Separator           | String between the first and second part of the daterange to format the Collections/MIGX value.                                                                          | "&thinsp;–&thinsp;"       |
